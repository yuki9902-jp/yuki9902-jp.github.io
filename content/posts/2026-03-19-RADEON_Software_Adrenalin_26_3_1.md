---
title: "RADEON Software Adrenaline 26.3.1"
date: 2026-03-20T17:00:00+09:00
archives: ["2026-03"]
categories: ['COMPUTER']
tags: ['RADEON','RX9070XT','Adrenalin','ROCm','Windows11Pro']
---

### [AMD Software: Adrenalin Edition 26.3.1 Release Notes](https://www.amd.com/en/resources/support-articles/release-notes/RN-RAD-WIN-26-3-1.html)

AMD Software: Adrenaline Edition 26.3.1 が公開されています。

新しいゲームサポートと目玉は新技術 RADEON 9000シリーズ向け FSR Upscaring 4.1 のサポートでしょうか。

ROCm7.2から変わってないかを確認しないといけないですね。今日現在（2026/03/20）はROCm7.2のままでインストール手順書が変わっていないので、変更はないと思いますが。

[Use ROCm on Radeon and Ryzen](https://rocm.docs.amd.com/projects/radeon-ryzen/en/latest/)
