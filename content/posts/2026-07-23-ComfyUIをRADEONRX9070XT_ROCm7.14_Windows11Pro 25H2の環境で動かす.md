---
title: "ComfyUIをRADEON RX9070XT(ROCm7.14)とWindows11Pro 25H2の環境で動かす"
...
2026/07/16にAMDからROCm7.14がリリースされました。また2026/06/29にAMD RadeonのドライバースイートであるAMD Software: Adrenaline Edition 26.6.4 (WHQL Recommended)も公開されました。
既にROCm7.2.1での環境を作っていますが、Pythonのvenvを使うようにAMDの文書が変更されているので、ROCm7.2.1とROCm7.14の環境を共存させてみます。
...
#### [Install PyTorch for ROCm 7.14.0](https://rocm.docs.amd.com/projects/ai-ecosystem/en/latest/frameworks/pytorch/install.html?fam=radeon&os=windows&pytorch-ver=2.12.0&i=pip&w=compute&gpu=rx-9070-xt&gfx=gfx1201)

1. [Install PyTorch for ROCm 7.14.0](https://rocm.docs.amd.com/projects/ai-ecosystem/en/latest/frameworks/pytorch/install.html?fam=radeon&os=windows&pytorch-ver=2.12.0&i=pip&w=compute&gpu=rx-9070-xt&gfx=gfx1201) に従いPyTorchをインストールします。管理者権限でコマンドプロンプトを起動します。
...
2. [Install ComfyUI](https://rocm.docs.amd.com/projects/radeon-ryzen/en/docs-7.2.1/docs/advanced/advancedrad/windows/comfyui/installcomfyui.html#install-comfyui) に従いComfyUIをインストールします。(通常のGit版のComfyUIのインストールです。ROCm環境だからと言って特別な方法ではありません。)
...
Windowsネイティブ環境で、ROCｍ7.2.1でもROCm7.14でも速度はほとんど変わりません。両方ともかなり良い感じで生成できます。GeForce環境がないので比較はしたことありませんが、そこそこいけるレベルではないでしょうか。
...
Windows11 25H2、AMD RADEON RX9070XT(16GB)、ComfyUI、ROCm7.14の環境で、上手く処理させれば一日で約5000枚の画像を生成することができます。
